...
cond13=repmat(11,1,trialscond);cond14=repmat(12,1,trialscond);
cond15=repmat(11,1,trialscond);
...
str5='your time to spot the patch. Then, push the LEFT BUTTON';
str6='to increase or the RIGHT BUTTON to decrease the contrast';
str7='until you are happy that the patch is set to a level ';
str8='that is just one button press above not being visible';
str9='at all (i.e. it is just barely visible).';
str10=' ';
str11='Press the TOP button when you have completed the';
str12='settting and the experiment will move on to the next';
str13='patch, where you repeat the task. ';
str14=' ';
str15='Press the LEFT button to start.';
...
trials_per_block = 45; %make a break halfway through
...
for j = 1:length(saveconds)
    ...
    if j == n * trials_per_block + 1
        ...
        %Select a page in memory to draw onto.
        crsSetDrawPage(CRS.VIDEOPAGE,6, backpix);
        crsDrawString([10 82],'Take a short break, then press any key to continue');
        %Display the page that the text was drawn onto.
        crsSetZoneDisplayPage(CRS.VIDEOPAGE,6);
...
if currentcond<6 %lum
  %adjust lum to L-M
       ...
       relLum1=90; th1=0;
        relLum2=-90; th2=0; 
        testRadius=0.01; %initial radius, when stim appears
...
% -----------------------------------------------------------------------------
 % Gabor parameters (in degrees)
  ...
  gabor_phase      = 0; %TO DO - set off centre 
  gabor_spatfr= SF;
...
% %now turn all those colours into RGB and put them into the LUT buffer
 for colnum=1:gabor_pixHi
      [LUTBuff(colnum+gabor_pixHi,:) ErrorCode]= ctGetColourTrival('CS_DKL','CS_RGB',[WP_RGB',STC1(colnum,:)],deviceSPD,Sensors);
             if ErrorCode == -1, disp('THE REQUESTED COLOUR IS OUT OF RANGE'); end
 end
