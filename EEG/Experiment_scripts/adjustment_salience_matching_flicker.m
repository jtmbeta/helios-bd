function adjustment_salience_matching_flicker(subject,viewdist,TF)
...
%
% this version allows TF to be set, for temporally modulated stimuli
%
...
%
%written by j martinovic, march 2024
%******************************************************************************

pathname='\research\wellcome\results\salienceTF\';
...
LUTBuff1(255,:)=white; %white
LUTBuff1(254,:)=[0,0,0]; %black
backpix=256; %set background to 256
LUTBuff2(255,:)=white; %white
LUTBuff2(254,:)=[0,0,0]; %black
whitepix=255;

crsLUTBUFFERWrite(1,LUTBuff1);     % load the CLUT into VSG LUT memory
...
% each trial is recorded in a single line in this file
  ch=exist(sprintf(sprintf('c:/research/wellcome/results/salienceTF/%s_matching.result',subject)));
  if ch==0
  fid1=fopen(sprintf('c:/research/wellcome/results/salienceTF/%s_matching.result',subject),'w');
  else
...
% %now turn all those colours into RGB and put them into the LUT buffer
 for colnum=1:gabor_pixHi
      [LUTBuff1(colnum,:) ErrorCode]= ctGetColourTrival('CS_DKL','CS_RGB',[WP_RGB',STC1(colnum,:)],deviceSPD,Sensors);
...
% %now turn all those colours into RGB and put them into the LUT buffer
 for colnum=1:gabor_pixHi
      [LUTBuff1(colnum+gabor_pixHi,:) ErrorCode]= ctGetColourTrival('CS_DKL','CS_RGB',[WP_RGB',STC2(colnum,:)],deviceSPD,Sensors);
             if ErrorCode == -1, disp('THE REQUESTED COLOUR IS OUT OF RANGE'); end
 end
