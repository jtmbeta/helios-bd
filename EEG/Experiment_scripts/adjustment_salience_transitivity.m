function adjustment_salience_transitivity(subject,viewdist)

%******************************************************************************
% adjustment to check the transitivity of S-(L+M) and L+M defined gabors
% set to equal salience in adjustment_salience_matching 
%
% S-(L+M) contrasts are fixed and lum adjusted to match them 
%
...
%----------------------------------------
 %each trial is recorded in a single line in this file
  ch=exist(sprintf(sprintf('c:/research/wellcome/results/salience/%s_transit.result',subject)));
  if ch==0
  fid1=fopen(sprintf('c:/research/wellcome/results/salience/%s_transit.result',subject),'w');
  else
...
%-----------------------------------------------------------------
trialscond=6;
cond1=ones(1,trialscond);cond2=repmat(2,1,trialscond); %match lum to S, 4 levels
...
save([pathname '\' subject '_transit_conds.txt'], 'saveconds', '-ASCII');
...
%contrasts to use should have been loaded as Scs and Lumcs
if currentcond==1
        radius=Scs(1); %for stand      
            testRadius=Lumcs(1); %for comp - initial
elseif currentcond==2
    radius =Scs(2);
    testRadius=Lumcs(2); %for comp - initial
